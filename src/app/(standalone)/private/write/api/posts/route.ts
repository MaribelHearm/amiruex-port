...
export async function GET(req: Request) {
  try {
    ...
    return Response.json({ posts: result.docs.map(postToWriterSummary) })
  } catch (error) {
    if (error instanceof Error && error.message === 'UNAUTHENTICATED') {
      return unauthenticatedResponse()
    }

    console.error('[writer-posts:get]', error)
    return Response.json({ error: '读取文章失败' }, { status: 500 })
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    return Response.json({ post: postToWriterDocument(post) }, { status: 201 })
  } catch (error) {
    if (error instanceof Error && error.message === 'UNAUTHENTICATED') {
      return unauthenticatedResponse()
    }
    ...
    console.error('[writer-posts:post]', error)
